...
for embeddedLength in embeddedLengthList:
	for r in rList:
		for concreteCoverRatio in concreteCoverRatioList:
			...
			zAxis = [[0]*3 for i in range(8)]
			for i in range(0, 8):
				point[i] = [x * r for x in [0, -math.sin(math.radians(i*45)), math.cos(math.radians(i*45))]]
				midPoint[i] = [x * r for x in [xLength/(4.*r), -math.sin(math.radians(i*45+45/2.)), math.cos(math.radians(i*45+45/2.))]]
				xAxis[i] = [0.0, -math.cos(math.radians(i*45+45/2.)), -math.sin(math.radians(i*45+45/2.))]
				zAxis[i] = [0.0, math.sin(math.radians(i*45+45/2.)), -math.cos(math.radians(i*45+45/2.))]
			...
			for i in range(0, 8):
				...
				#setParameter( "GEOMET", "Int"+str(i), "LOCAXS/ZAXIS", zAxis[i] )
			...
			for i in range(8, 12):
				...
				#setParameter( "GEOMET", "Int"+str(i), "LOCAXS/ZAXIS", zAxis[i - 8] )
			...
			#setParameter( "GEOMET", "WoodCon-Interface", "LOCAXS/XAXIS", [0, -1, 0] )
			#setParameter( "GEOMET", "WoodCon-Interface", "LOCAXS/ZAXIS", [0, -1, 0] )
			...
			#em = Embedded length
			path = "Model-em"+str(embeddedLength*1000)[:-2]+"-r"+str(r*1000)[:-2]+"-rB"+str(rBig*1000)[:-2]
