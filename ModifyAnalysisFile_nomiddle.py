...
def modAnalysisFile(dir):
	with open('GeneralAnalysisfile-nomiddle.dcf', 'r') as file :
		filedata = file.read()
		file.close()
	...
	for i in range(len(files)):
		...
		filedataTempWindows = filedataTempWindows.replace('%%DisplacementReinfo1%%', nodeNumbers[3])
		filedataTempLinux = filedataTempLinux.replace('%%DisplacementReinfo1%%', nodeNumbers[3])
		
		filedataTempWindows = filedataTempWindows.replace('%%DisplacementConReinfo1%%', nodeNumbers[4])
		filedataTempLinux = filedataTempLinux.replace('%%DisplacementConReinfo1%%', nodeNumbers[4])
...
def addNodes(filename):
	...
	reinfo1Center = instance[3][3:-2]
	concretereinfo1Points = instance[4][3:-2]
	return([supportTop, yMiddleNode, reinfo1Debond, reinfo1Center, concretereinfo1Points])